...
train = pd.read_csv('train_data_2013_2018.csv')
trade = pd.read_csv('trade_data_2019_2023.csv')
#print(len(train))
#print(len(trade))
...
def add_sentiment(train, sentiment): #column_name=['Sentiment', 'Probability_Se']):
    ...
    sentiment_small.rename(
        columns={'Stock_symbol': 'tic', 'Sentiment': 'llm_sentiment', 'Probability_Se':'Confidence_Sen'},
        inplace=True
    )
...
def add_risk(train, risk): # column_name=['Risk', 'Probability']):
    train_new = train
    risk_new=risk
    train_new['date'] = pd.to_datetime(train_new['date'])
    ...
    # Step 3: Rename 'Stock_symbol' to 'tic' and the sentiment column
    risk_new.rename(
        columns={'Stock_symbol': 'tic', 'Risk': 'llm_risk', 'Probability':'Confidence_Risk'},
        inplace=True
    )
...
train_risk.to_csv('train_risk_2013_2018.csv')
trade_risk.to_csv('trade_risk_2019_2023.csv')
