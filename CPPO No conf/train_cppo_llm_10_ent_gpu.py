...
from env_trading_cppo_llm10 import StockTradingEnv
...
train = pd.read_csv('drive/MyDrive/FinRL_DeepSeek/train_data_deepseek_risk_2013_2018_trial.csv')
...
# Set this as the index
train = train.set_index('new_idx') """
#,date,tic,close,high,low,open,volume,day,macd,boll_ub,boll_lb,rsi_30,cci_30,dx_30,close_30_sma,close_60_sma,vix,turbulence,llm_sentiment,Confidence_Sen,llm_risk,Confidence_Risk
...
# state_space = balance + stocks(shares and prices for every stock) + indicators(indicators for every stock, llm sentiment and llm risk for every stock)
state_space = 1 + 2*stock_dimension + (4+len(INDICATORS))*stock_dimension  #add dimensions for LLM sentiment and risk
...
class Actor(nn.Module):
    ...
    def forward(self, obs, act=None):
        ...
        if torch.isnan(obs).any() or torch.isinf(obs).any():
            raise ValueError("Invalid values in obs: NaN or Inf detected")
...
class MLPGaussianActor(Actor):
    ...
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation):
        ...
        #self.log_std = torch.nn.Parameter(torch.clamp(torch.as_tensor(log_std), min=-20, max=2))
...
def cppo(env_fn,
         ...
         device=None):  # CVaR clipping ratio
    ...
    def compute_loss_pi(data):
        ...
        # Get LLM score from observation (if added to state)
        llm_score = obs[0, -2*stock_dimension:-stock_dimension]  # Assume last feature is LLM score
        llm_score_conf = obs[0, -stock_dimension:]
       # print('llm score', llm_score, 'llm conf ==', llm_score_conf)
        if not torch.all(llm_score == 0) and not torch.all(llm_score_conf == 0):
            entropy_coef = (5 - llm_score) * llm_score_conf ** 2  # Higher score → less entropy
            entropy_bonus = entropy_coef * pi.entropy().mean()
            # Repeat entropy bonus along the batch dimension
            #entropy_bonus = entropy_bonus.repeat(loss_pi.shape[0])

            loss_pi -= entropy_bonus.mean()  # Encourage less exploration for high scores
...
#            llm_risks_weights = llm_risks_weights.flatten() 
    #        +    # New dictionary with wider range
            risk_to_weight = {1: 0.90, 2: 0.95, 3: 1.00, 4: 1.05, 5: 1.10}
            llm_risks_raw = next_o[0, -3*stock_dimension:-2*stock_dimension]  # raw risk in [1..5]
            #print('llm_risks_raw == ', llm_risks_raw)
            
            #print("llm_risks_raw:", llm_risks_raw)  # Debugging output
            base_risk_factors = np.vectorize(risk_to_weight.get)(llm_risks_raw)
            confidence_risk = next_o[0, -stock_dimension:]  # confidence in risk
            #print("confidence_risk:", confidence_risk)  # Debugging output
            # Adaptive formula: (R_f^i)^{mod} = 1 + (R_f^i - 1)*BETA
            BETA = 0.5  # user-defined
            R_f_mod = 1.0 + (base_risk_factors - 1.0) * confidence_risk * BETA
            #print("R_f_mod:", R_f_mod)
     ...
     #       print("llm risks weights are  : " + str(llm_risks_weights))
      ...
      #      print("prices length is: " + str(prices.shape))
            ...
            # Renormalize the stock values so that their sum is 1
                ...
                llm_risk_factor = np.dot(stock_weights, R_f_mod)
...
# Save the model
model_path = TRAINED_MODEL_DIR + "/agent_cppo_llm10_ent_20_epochs_20k_steps_trial_gpu_v2.pth"

torch.save(trained_cppo.state_dict(), model_path)
torch.save(trained_cppo.state_dict(), "drive/MyDrive/FinRL_DeepSeek/trained_models/agent_cppo_llm10_ent1_20_epochs_20k_steps_trail_gpu.pth")
