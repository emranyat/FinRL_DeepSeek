...
from env_trading_cppo_ent import StockTradingEnv
...
def cppo(env_fn,
    ...
    def compute_loss_pi(data):
        ...
        llm_score = obs[0, -4*stock_dimension:-3*stock_dimension]  # LLM score
       ...
       # print('llm score', llm_score, 'llm conf ==', llm_score_conf)
        if not torch.all(risk_score == 0) and not torch.all(risk_score_conf == 0):
            entropy_coef = 0.01 * (risk_score) * risk_score_conf  # Higher score → less entropy
...
# Save the model
model_path = TRAINED_MODEL_DIR + "drive/MyDrive/FinRL_DeepSeek/trained_models/agent_cppo_ent1_20_epochs_20k_steps_trail_gpu.pth"
