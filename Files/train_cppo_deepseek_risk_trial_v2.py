...
from env_stocktrading_cppo_risk_trial_v2 import StockTradingEnv
...
train = pd.read_csv('/homes/eya24/projects/FinRL_DeepSeek/train_data_deepseek_risk_2013_2018_trial.csv')
...
train[ 'Confidence_Sen'].fillna(0.5, inplace=True) #0 is outside scope of sentiment scores (min is 1)

train['llm_risk'].fillna(3, inplace=True) #neutral risk score is 3
train[ 'Confidence_Risk'].fillna(0.5, inplace=True) #neutral risk score is 3
...
#            llm_risks_weights = llm_risks_weights.flatten() 
    #        +    # New dictionary with wider range
            ...
            llm_risks_raw = next_o[0, -stock_dimension:]  # raw risk in [1..5]
            base_risk_factors = np.vectorize(risk_to_weight.get)(llm_risks_raw)
            confidence_risk = next_o[0, -2*stock_dimension:-stock_dimension]  # confidence in risk
...
# Save the model
model_path = TRAINED_MODEL_DIR + "/agent_cppo_deepseek_20_epochs_20k_steps_trial.pth"
