...
columns_to_load = ['Date', 'Stock_symbol', 'Sentiment', 'Probability_Se']
sentiment = pd.read_csv('combined_risk_df.csv', usecols=columns_to_load)
#sentiment = pd.read_csv('sentiment_qwen_nasdaq_news_full.csv', usecols=columns_to_load)

print(sentiment.head())


columns_risk = ['Date', 'Stock_symbol', 'Risk', 'Probability']

risk = pd.read_csv('combined_risk_df.csv', usecols=columns_risk)
...
def add_sentiment(train, sentiment, column_name='Sentiment'):
    sentiment_small = sentiment.copy()
    train_new = train
    ...
    sentiment_small.rename(
        columns={'Stock_symbol': 'tic', column_name: 'llm_sentiment', 'Probability_Se': 'Confidence_Sen'},
        inplace=True
    )

    # Step 4: Merge train and sentiment_small into a new DataFrame
    train_new = train.merge(
        sentiment_small[['Date', 'tic', 'llm_sentiment', 'Confidence_Sen']],
        left_on=['date', 'tic'],
        right_on=['Date', 'tic'],
        how='left'
    )
...
def add_risk(train, risk, column_name='Risk'):
    train_new = train
    risk_new = risk.copy()
    train_new['date'] = pd.to_datetime(train_new['date'])
    ...
    # Step 3: Rename 'Stock_symbol' to 'tic' and the sentiment column
    risk_new.rename(
        columns={'Stock_symbol': 'tic', column_name: 'llm_risk', 'Probability': 'Confidence_Risk'},
        inplace=True
    )

    # Step 4: Merge train and sentiment_small into a new DataFrame
    train_new = train.merge(
        risk_new[['Date', 'tic', 'llm_risk', 'Confidence_Risk']],
        left_on=['date', 'tic'],
        right_on=['Date', 'tic'],
        how='left'
    )
...
print(train.shape)
print(sentiment[['Date', 'tic', 'llm_sentiment', 'Confidence_Sen']].shape)

train_risk.to_csv('train_data_deepseek_risk_2013_2018_trial.csv')
trade_risk.to_csv('trade_data_deepseek_risk_2019_2023_trial.csv')
