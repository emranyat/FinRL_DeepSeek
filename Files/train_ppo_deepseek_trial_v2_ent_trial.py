...
from env_stocktrading_ppo_trial_v2 import StockTradingEnv
...
train['Confidence_Sen'].fillna(0, inplace=True)
...
def ppo(
    ...
    epochs=20,  # More epochs for convergence in a volatile environment
  ...
  #      vf_lr=1e-3, train_pi_iters=80, train_v_iters=80, lam=0.97, max_ep_len=1000,
   #     target_kl=0.01, logger_kwargs=dict(), save_freq=10):  
    ...
    def compute_loss_pi(data):
        ...
        llm_score = obs[:, -2]  # Assume last feature is LLM score
        llm_score_conf = obs[:, -1]
        ...
        if not torch.all(llm_score == 0) and not torch.all(llm_score_conf == 0):
            ...
            loss_pi -= entropy_bonus  # Encourage less exploration for high scores

       
        # Get LLM score from observation (if added to state)
        llm_score = obs[:, -1]  # Assume last feature is LLM score
        entropy_coef = 0.01 * (5 - llm_score)  # Higher score → less entropy
...
parser.add_argument('--epochs', type=int, default=20) # 10 for test otherwise 100 it is too much 150)  # Updated to match epochs in ppo
...
# Save the model
model_path = TRAINED_MODEL_DIR + "/agent_ppo_deepseek_20_epochs_20k_steps_ent_trial.pth"
